...
class IndexController extends BaseController
{
    ...
    public function CollectExpression(){
      ...
      if($postion)
      {
          $strsrc=substr($url,$postion,strlen($url)-$postion);
          $miniblog_name=substr($strsrc,strrpos($strsrc,'/')+1,strlen($strsrc)-strrpos($strsrc,'/'));
          $miniblogname = substr($miniblog_name, 0, strrpos($miniblog_name, "."));
          $miniblogpath='/Application/Core/Static/images/expression/miniblog/'.$miniblog_name;
         // $this->ajaxReturn($miniblogpath);
        $map['uid'] = $uid;
          $map['path'] = $miniblogpath;
          $result = $iexp->where($map)->find();
          if ($result) {
              $this->ajaxReturn('1');
          } else {
              $data['name'] = $miniblogname;
              $data['path'] = $miniblogpath;
              $data['driver'] = 'network';
              $data['from'] = 'miniblog';
              $data['create_time'] = time();
              $data['uid'] = is_login();
              $res = $iexp->data($data)->add();
              $this->ajaxReturn('2');
          }
      }else {
          $len = strlen($url);
          ...
          $str = str_replace('/Uploads/Expression/', '', $path);
          $from = substr($str, 0, strpos($str, '/'));
          $name = substr($str, strpos($str, '/') + 1, strlen($str) - strpos($str, '/'));
